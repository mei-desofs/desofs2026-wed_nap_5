...
class ChatServiceTest {
    ...
    void shouldSendCourseQuestionSuccessfully() {
        ...
        savedMessage.setContent("Professor, could you clarify the assignment deadline?");
        savedMessage.setSentAt(new Date());

        when(enrollmentService.isUserEnrolled(userId, chatRoomId)).thenReturn(true);
        ...
        assertEquals("Professor, could you clarify the assignment deadline?", response.getContent());
    ...
    void shouldRejectUnauthorizedCourseAccess() {
        ...
        when(chatRoomRepository.findById(chatRoomId)).thenReturn(Optional.of(room));
        when(enrollmentService.isUserEnrolled(userId, chatRoomId)).thenReturn(false);
    ...
    void shouldSanitizeMaliciousScriptInjection() {
        ...
        savedMessage.setContent("&lt;script&gt;fetch('http://attacker.com')&lt;/script&gt;");
        savedMessage.setSentAt(new Date());
        ...
        assertEquals("&lt;script&gt;fetch('http://attacker.com')&lt;/script&gt;", response.getContent());
    }
    ...
    void shouldReturnOrderedDiscussionMessages() {
        ...
        when(chatMessageRepository.findByChatRoomIdOrderBySentAtAsc(eq(chatRoomId), any(Pageable.class))).thenReturn(page);

        Page<ChatMessageResponse> responses = chatService.getMessages(chatRoomId, Pageable.unpaged());

        assertEquals(2, responses.getContent().size());
         
        assertEquals("Does anyone understand exercise 3?", responses.getContent().get(0).getContent());
        assertEquals("Yes, the professor explained it in class yesterday.", responses.getContent().get(1).getContent());
    }
